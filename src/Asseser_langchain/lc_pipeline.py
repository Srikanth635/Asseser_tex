...
# Import the desired vector database
from langchain_community.vectorstores import FAISS  # Example: using FAISS

## setting up directories
current_dir_path = os.path.dirname(
    os.path.abspath(__file__))  ## <- extracting the directory name from the absolute path of this file
data_path = os.path.join(current_dir_path,".." ,"files")  ## creating a path for the `data` folder
persistent_directory = os.path.join(current_dir_path,
                                    "data-ingestion-local")  ## creating a directory to save the vector store locally
...
if not os.path.exists(persistent_directory):
    ...
    for pdf in pdfs:
        ...
        for doc in docsRaw:
            doc_container.append(
                doc)  ## <- appending each `Document` object to the previously declared container (list)
    ...
    """
    vectorDB = FAISS.from_documents(  # Use FAISS instead of Chroma
